...
class WaypointManager(Node):
    def __init__(self):
        ...
        self.waypoints = {
            "w_start": Waypoint(name="w_start", id=0, x=0.0, y=0.0),
            "w0": Waypoint(name="w0", id=0, x=-7.0, y=1.5),
            "w1": Waypoint(name="w1", id=0, x=-3.0, y=-8.0),
            "w2": Waypoint(name="w2", id=0, x=6.0, y=2.0),
            "w3": Waypoint(name="w3", id=0, x=7.0, y=-5.0),
            "w_end": Waypoint(name="w_end", id=0, x=0.0, y=0.0),
        }
    ...
    def update_waypoint_id_callback(self, request, response):
        ...
        if waypoint:
            ...
            response.success = True
            response.msg = f"Waypoint '{request.name}' updated to ID:{request.id}."
    ...
    def update_w_end(self):
        ...
        self.get_logger().info(f"Waypoint 'w_end' updated to match '{lowest_waypoint.name}' with ID:{lowest_waypoint.id}.")
